...
3. Add a **Domain** pointed at the `outreachemailmcp` service, container port **3000**, HTTPS enabled (Dokploy's Traefik terminates TLS). Then **delete the `3000:3000` port mapping** from the compose to avoid host-port conflicts.
4. Keep the `2525` (SMTP) and `1143` (IMAP) port mappings — they're raw TCP that Traefik doesn't route — and open those ports in your server's firewall. These are what sequencers connect to, so in the proxy's env the advertised host comes from `BASE_URL`'s hostname.
